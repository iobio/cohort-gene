...
<template>
	<div>
		<div class="variant-viz loaded-variant-viz" id="selectionDetailsLine">
			<span class="field-label-header mr-2">Selection Details:</span>

			<!-- phenotypic data -->
			<v-chip disabled small v-for="phenotype in phenotypes"
					:key="phenotype">
				{{phenotype}}
			</v-chip>
		</div>
		<div class="variant-viz mt-2 mb-5" id="sourceFileLine">
			<span class="field-label-header mr-2">Analysis Sources:</span>
			<v-chip disabled small v-for="file in validSourceFiles"
					:key="file">
				{{file}}
				<v-icon right color="cohortDarkBlue">check_circle</v-icon>
			</v-chip>
			<template v-for="(file,index) in invalidSourceFiles">
				<v-tooltip bottom>
					<v-chip disabled small style="font-size: 12px; font-family: 'Open Sans'" slot="activator"
							:key="file">
						{{file}}
		...
		>
			<v-chip disabled color="red" small outline>
				No Variants Found
			</v-chip>
		</div>
	</div>
