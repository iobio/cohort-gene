...
<v-card tile id="gene-track" class="app-card">
    ...
    <div style="text-align:center;display:inline-block;width:100%;padding-top:5px">
      <div style="vertical-align:top;display:inline-block">
        ...
        <div id="region-buffer-box" style="display:inline-block;width:50px" v-if="showGene">
          <v-text-field id="gene-region-buffer-input" class="sm level-edu level-basic  fullview" v-model="regionBuffer" v-on:change="onGeneRegionBufferChange">
          </v-text-field>
        </div>
      ...
      <div id="gene-source-box" v-if="showGene" style="margin-top:-7px;margin-left:20px;display:inline-block;width:150px">
        <v-select v-bind:items="geneSources" v-model="geneSource" label="Gene source" class="input-group--focused" item-value="text" @input="onGeneSourceSelected">
        </v-select>
      </div>

      <transcripts-viz v-bind:class="{ hide: !showGene }" :selectedGene="selectedGene" :selectedTranscript="selectedTranscript" @transcriptSelected="onTranscriptSelected">
      </transcripts-viz>
  ...
  <div id="non-protein-coding" class="level-edu level-basic">
    ...
    <div id="gene-type-badge" v-if="showGeneTypeWarning" class="label label-warning" style="display:block;margin-bottom:2px;">
      {{ selectedGene.gene_type }}
    </div>
    <div id="transcript-type-badge" v-if="showTranscriptTypeWarning" class=" label label-warning" style="display:block;">
      {{ selectedTranscript | formatTranscriptType }}
    </div>
    <div id="no-transcripts-badge" v-if="showNoTranscriptsWarning" class=" label label-warning" style="display:block;">
  ...
  <div id="transcript-panel" class="level-edu fullview">
    ...
    <gene-viz id="gene-viz" :data="[selectedTranscript]" :margin="margin" :height=40 :trackHeight="trackHeight" :cdsHeight="cdsHeight" :regionStart="parseInt(selectedGene.start)" :regionEnd="parseInt(selectedGene.end)" :showBrush=true @region-zoom="onRegionZoom"
